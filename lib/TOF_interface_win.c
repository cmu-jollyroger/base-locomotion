...
#define STMCOM_TTY "/dev/tty.usbmodem1453"
...
static int set_interface_attribs (int speed, int parity) {
  printf("In set_interface_attribs.\n");
  struct termios tty;
...
int stmcom_init(void) {
  ...
  printf("Ready to set interface attribs.\n");
  tty_file = fopen (STMCOM_TTY, "r");
  printf("Ready to set interface attribs.\n");
  if (tty_file == NULL) {
    printf( "error opening usbtty\n");
  }
  printf("Ready to set interface attribs.\n");
